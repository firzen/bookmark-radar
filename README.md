...
- **全量扫描 + 并发** — 读取所有书签，1/3/5/10 个标签页并发检查，工具栏角标实时显示进度百分比
- **智能缓存** — 每个书签独立 30 天缓存，二次扫描秒出；目录页（已提取章节）/超时/验证页不缓存，每次重查以追踪更新；勾选「强制」清除缓存重扫
...
- **Cloudflare 应对** — 自动识别验证页；勾选「真人验证」时临时前台激活标签等 JS 质询通过（后台标签 rAF 被暂停无法自动过），也可点链接手动过验证，cookie 全域名生效
- **数据导出** — 复制文本 / 导出 JSON
...
├── background/
│   └── service-worker.js      # 扫描调度核心（并发 + 缓存 + 角标）
├── content/
│   └── extractor.js           # 页面注入脚本（验证页检测 + 目录识别）
├── shared/
│   └── renderer.js            # 报告渲染共享逻辑（popup 与报告页共用）
├── popup/                     # 弹窗界面（扫描控制 + 章节快捷列表）
├── report/                    # 完整报告页（新标签页全宽查看）
└── icons/
```
